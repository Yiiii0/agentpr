...
> 更新时间：2026-03-10
> 状态：D4 基本完成。**战略转向：从 pipeline 优化 → merge rate 优化。** Pipeline PASS rate 86% 已验证，但 merge rate 5.9%（1/17）才是真正指标。下一步：收割现有 PR + repo targeting + scale to 50。
...
5. `merge` 始终人工；`create PR` 受 gate 保护（human-confirm 或 Manager assessment 双模式）。
...
4. **D4 运维化基本完成。** Manager loop 常驻 + wake file 事件驱动 + 全局进度通知 + 双 PR 模式（human-confirm / auto-create with Manager assessment）已实装。批量创建 → 常驻处理 → GitHub 事件唤醒 → 自动推进 → 人只看通知。
5. **Manager LLM 是无记忆的单次调用。** 9 个 LLM 方法全部 1-shot，无跨 tick 推理连续性。当前 rules 处理 90%+ 路径所以够用，但距离 "Manager 是持续在线的大脑" 有结构性差距。
6. **Forge 422 仍阻塞**：使用 Codex 原生 provider，不影响核心管线。

**结论：Pipeline 技术闭环已完成。核心瓶颈已从"能不能跑通"转移到"PR 能不能被 merge"。Merge rate（5.9%）才是下一步的优化目标，不是 pipeline 功能。**
...
Orchestrator (薄层：状态持久化 + gate 执法 + 事件日志)
  |-- 硬约束：PR 创建需人工确认或 Manager assessment / merge 永远人工
...
**全部 9 个 LLM 方法均为无记忆的单次调用（1-shot）。** 没有多轮对话，没有跨 tick 记忆。

```
每次 tick:
  读 DB + artifacts → 构建 ManagerRunFacts（快照）→ 调 LLM（只看当前 facts）→ 执行 → 写回 DB

LLM 不知道：上一次 tick 做了什么决策、同一 run 的历史决策链、其他 run 的详情
LLM 知道的：当前 run 的 state/grade/evidence/artifacts + global_stats 摘要
```

| 特征 | 当前设计 |
|------|----------|
| **调用模式** | 全部 1-shot（system + user → response），无 conversation memory |
| **Tool-calling** | 8/9 方法用 function tool-calling（有 JSON fallback）；1 个纯文本生成（PR description） |
| **Temperature** | 全部 temp=0（确定性），仅 PR description temp=0.3 |
| **Stateless** | 每次调用独立，可重启，可恢复。记忆完全依赖 DB artifacts |
| **Guardrail** | hybrid 模式：LLM 可降级（active → WAIT_HUMAN），不可升级（rules 优先） |

**优点**：无状态，可重启，tick 解耦，failure-safe。
**缺点**：Manager 无推理连续性，无法做"上次试了 X 不行，换 Y"的跨 tick 推理。
**当前够用**：Rules engine 处理 90%+ 确定性路径，LLM 做边界判断。

### 5.1 应使用 LLM（9 个方法，已完成）

| # | 方法 | 调用位置 | 用途 | 输入 |
|---|------|----------|------|------|
| 1 | `decide_action` | Manager Loop tick | 状态机决策（从 allowed_actions 选） | run facts + global stats |
| 2 | `grade_worker_output` | Runtime grading / Loop | Worker 输出语义分级 | evidence 包（test/lint/diff） |
| 3 | `explain_decision_card` | Telegram `/show` `/status` | 人类可读的决策解释 | decision card |
| 4 | `decide_bot_action` | Telegram NL 路由 | 自然语言→命令映射 | user text + recent runs |
| 5 | `triage_review_comment` | Manager Loop ITERATING | PR review 评论分流 | comment body + run context |
| 6 | `suggest_retry_strategy` | Manager Loop FAILED | 失败诊断 + 重试策略 | failure evidence |
| 7 | `generate_pr_description` | CLI request-open-pr | 生成 diff-aware PR body | diff + stat + evidence + PR template |
| 8 | `review_code_changes` | CLI run-code-review | 深度代码审查 | diff + changed files + siblings + checklist |
| 9 | `assess_pr_readiness` | Manager Loop AUTO_CREATE_PR | PR 就绪度最终评估 | code review + evidence + PR body + template + diff + 7 principles |
...
### Manager LLM 能力（9 方法，全部 1-shot 无记忆）
- 状态机决策：`decide_action()`（hybrid 模式 rules+LLM，guardrail 保护）
- 语义分级：`grade_worker_output()`（rules 证据 + LLM → PASS/NEEDS_REVIEW/FAIL + confidence）
- Confidence routing：低信心 PASS 升级人工审核
- Decision Card：`explain_decision_card()` → why_machine + why_llm + suggested_actions 双层展示
- NL 路由：`decide_bot_action()` → 自然语言→命令（rules/hybrid/llm 三模）
- Review triage：`triage_review_comment()` → fix_code/reply_explain/ignore
- 失败诊断：`suggest_retry_strategy()` → should_retry + target_state + 修改指令
- PR Description：`generate_pr_description()`（diff + evidence + PR template → 智能填充）
- Code Review：`review_code_changes()`（diff + changed files + siblings + checklist → CLEAN/HAS_ISSUES）
- PR 就绪评估：`assess_pr_readiness()`（code review + evidence + PR body + template + 7 principles → APPROVE/NEEDS_HUMAN）
- 全局统计：get_global_stats 接入 /overview（pass_rate、grade 分布、top reason_codes）
- 通知：manager_notification artifact → Telegram 推送（含优先级标记）

### D4 运维化能力（2026-03-10）
- **Manager Loop 常驻**：`--persistent` daemon，idle 时 hibernate（600s），无 idle exit
- **事件驱动**：`.wake_manager` 信号文件，Webhook/Bot 写 → Manager 读并唤醒（1s 响应）
- **全局进度通知**：每 300s 推送批量摘要（hash 变更检测 + batch complete + NEEDS_HUMAN callout）
- **双 PR 模式**：Mode 1 human-confirm（默认）/ Mode 2 auto-create with Manager PR Readiness Assessment（`AGENTPR_AUTO_PR=true`）
- **Auto-title**：`--title` 可选，空时自动生成
...
**当前状态：✅ 已达成。** NL → create_run → auto-prepare → manager loop → PUSHED/gate 全通路。D1.6 修复了 dirty workspace 误升级和 guardrail 问题。
**运维侧：✅ 已验证。** D2 Test Run 6: 5/5 PASS+PUSHED。D3 Test Run 7: 4/5 PASS+PUSHED tick 1。涵盖 Python + TypeScript + 混合项目。
**常驻化：✅ D4 已完成。** `--persistent` daemon + wake file 事件驱动 + 双 PR 模式（human-confirm / auto-create with Manager assessment）。

### 目标能力 B：问"现在什么情况"→ 全局态势 + 下一步

**当前状态：基本达成。** `/overview` + `/show` + Decision Card 双层展示 + 全局进度通知（每 300s 批量摘要）。
**缺口**：缺"下一步优先级队列"——自动告诉用户"最值得关注的 3 件事"。

### 目标能力 C：状态变更时主动通知

**当前状态：✅ 已达成。** Bot 关键状态通知 + manager_notification 推送 + 全局进度通知 + batch complete 检测 + NEEDS_HUMAN callout。D1.6 加了 stale state 检测（CI_WAIT >24h / REVIEW_WAIT >48h 自动升级）。
...
### 目标能力 F：Manager 作为持续在线的大脑

**当前状态：⚠️ 有差距。** Manager 有 9 个 LLM 能力（决策/分级/解释/NL路由/review分流/失败诊断/PR生成/代码审查/PR就绪评估），但**全部是无记忆的单次调用**。每次 tick 从 DB 重新构建 facts，LLM 不知道上次决策是什么。
**影响**：Manager 是"每次从零开始的顾问"，不是"持续跟进的大脑"。无法做跨 tick 推理（如"上次建议 X 不行，这次换 Y"）。
**当前够用**：Rules engine 处理 90%+ 确定性路径，LLM 做边界判断。但距离 North Star "Manager 是系统大脑" 有结构性差距。
**下一步方向**：per-run decision history（artifact 持久化）→ LLM 可以读到同一 run 的历史决策链，实现跨 tick 推理连续性。
...
| **D4：运维化（常驻 + 事件驱动 + 通知 + 双 PR 模式）** | **基本完成** | — |
| **D5：Merge Rate 优化（战略转向）** | **待开始** | **⭐ 最高** |
...
| 优先级 | 项目 | North Star | 状态 |
|--------|------|------------|------|
| ⭐ | Manager loop persistent + wake file | #2: Manager always online | ✅ D4.1 |
| ⭐ | 全局进度通知 | #1: Human in Telegram | ✅ D4.2 |
| ⭐ | 双 PR 模式 + Manager PR readiness assessment | #4: Safety by default | ✅ D4.3 |
...
| 中 | Self-iteration closed loop | #5: Self-improving | 🔲 |
...
**✅ 已完成：**
1. **Manager loop 常驻化** — `--persistent` daemon + hibernate（600s）+ wake file 事件驱动
2. **全局进度通知** — 每 300s 批量摘要 + batch complete 检测 + NEEDS_HUMAN callout
3. **双 PR 模式** — human-confirm（默认）/ auto-create with Manager PR Readiness Assessment

**🔲 剩余：**
4. **Bot 会话持久化**（run_id 绑定从内存态改为 SQLite）
5. **Review triage 用户确认**（triage 结果先问用户，再执行）
6. **优先级队列**（"你最应该关注的 3 件事"）
7. **自我迭代闭环**（skills-feedback → prompt/policy patch 草案 → 人审批）
8. **端到端验证**（D4 全部功能集成测试）

### D5（⭐ 最高优先）：Merge Rate 优化（战略转向）

**核心判断**：Pipeline 技术闭环已完成（86% PASS rate）。瓶颈从"能不能跑通"转移到"maintainer 会不会 merge"（当前 5.9%）。更多基础设施不会改变 merge rate。

| 优先级 | 项目 | 预期影响 |
|--------|------|---------|
| ⭐ | **收割现有 17 PRs**：签 CLA、bump 零回应 PR、跟踪 conversion funnel | 直接提升 merge 数 |
| ⭐ | **Repo targeting**：只选有 provider registry/factory 的 repo（数据证明质量最高） | 提升 PR 质量 → merge rate |
| 高 | **Scale to 50 repos**（用现有系统，不加新功能）：测量真实 merge rate | 获取 product-market fit 数据 |
| 高 | **Repo 可集成性评估**（新 LLM 方法）：preflight 阶段评估 repo 架构 → 跳过不适合的 | 避免低质量 PR |
| 中 | **PR follow-up 自动化**：3 天无回应 → 礼貌 bump comment | 提升 respond rate |
| 中 | `suggest_retry_strategy` 加 `previous_attempts` 历史 | 减少无效 retry |
| 低 | `decide_bot_action` 加对话历史 | 改善 Telegram UX |
| ~~低~~ | ~~Manager 推理连续性（per-run decision chain）~~ | ~~延期：分类器加记忆的 ROI 不够~~ |

**关键洞察（insight #60-64）：**
- PASS rate ≠ merge rate。优化错误指标是最大浪费
- 代码质量和 repo 架构强相关。Repo 选择策略直接决定产出质量
- 修 bug 的 PR 比加 provider 的 PR 更容易被 merge
- Manager LLM 9 个方法中只有 3 个高价值（#7 #8 #9），其余是分类器
- "给现有分类器加上下文"（低 ROI）≠ "让 Manager 做新事情"（高 ROI）

### D-forge：Forge 切回（等外部修复）

Forge 422 修复后验证并切回。独立于 D4/D5。
...
**D4 运维化 + 架构审视后：**
57. **Manager 无记忆的单次调用是当前最大的结构性限制。** 9 个 LLM 方法全部 1-shot，无跨 tick 推理。Manager 每次 tick 从 DB 重新构建 facts，LLM 不知道上次决策是什么。这使得 Manager 是"每次从零开始的顾问"而不是"持续跟进的大脑"。当前 rules engine 处理 90%+ 路径所以够用，但距离 North Star "Manager 是系统大脑" 有结构性差距。下一步：per-run decision history artifact，让 LLM 读到同一 run 的决策链。
58. **无状态设计的优点不应被低估。** 无状态 = 可重启、可恢复、tick 解耦、failure-safe。如果 Manager loop 崩溃重启，不丢失任何决策上下文（因为根本没有）。这比有状态 agent 的恢复成本低得多。改进方向应该是"给 LLM 更多 artifacts 作为上下文"，而不是"维护一个有状态的对话"。
59. **LLM 能力已覆盖全流程但深度不够。** 9 个方法覆盖了决策、分级、审查、生成、路由、诊断——几乎每个需要判断的环节都有 LLM。但每个调用都是浅层的（1-shot），没有深度推理或多步骤思考。当前够用是因为任务本身（"Forge 集成"）模式化程度高，一个 1-shot 判断就够了。更复杂的任务可能需要 agent-style 的多步推理。

**战略审视后（D4 完成时刻）：**
60. **PASS rate ≠ merge rate，优化错误指标是最危险的浪费。** 22 repos, 19 PASS (86%), 17 PRs submitted, 1 merged (5.9%). Pipeline 技术上 work，但 maintainer 不 merge。继续优化 pipeline 是 insight #34 的再次违反："不要继续堆代码。去跑真实测试。" 这次的"真实测试"不是"Worker 能不能跑通"，是"maintainer 会不会 merge"。
61. **代码质量和 repo 架构强相关（来自 pr_review_findings.md Problem 9）。** 有 registry/factory 抽象的 repo → Worker 只填参数 → 代码干净（ai-gradio, kit, DeepCode）。需要修改已有代码的 repo → Worker"创新" → 引入 bug（DAMO-ConvAI, weave）。**Repo 选择策略直接决定产出质量。**
62. **修 bug 的 PR 比"加新 provider"的 PR 更容易被 merge。** octotools #53 被 merge 是因为它修了一个真实 bug（factory routing）。对 maintainer 来说，修 bug = 即时价值，加 provider = 增加维护负担。如果 PR 不能证明对 repo 的即时价值，maintainer 没有动力 merge。
63. **Manager LLM 9 个方法中只有 3 个有高价值。** generate_pr_description、review_code_changes、assess_pr_readiness 直接影响 PR 质量。其余 6 个是分类器（PASS/FAIL、fix/ignore、retry/stop），rules + heuristic 已经够用。给分类器加"上下文记忆"的 ROI 远低于新增"repo 可集成性评估"或"PR follow-up 自动化"。
64. **"给现有方法加上下文"和"让 Manager 做新事情"是完全不同的问题。** 前者是优化分类器（低 ROI），后者是增加推理能力（高 ROI）。真正的"Manager 大脑"不是让 decide_action 记住上次选了什么，是让 Manager 能评估"这个 repo 适不适合自动 PR"、"这个 PR 需不需要 follow-up"、"下一批应该优先哪些 repo"。
...
### Phase D4：运维化 — 进行中（2026-03-10）

**D4.1 — 常驻化 + 事件驱动（已完成）：**
1. `cli.py`：`run-manager-loop --persistent` 模式 — idle 时不退出，改为 hibernate（600s）等待信号
2. `github_webhook.py`：处理 actionable events 后 touch `.wake_manager` 信号文件 — 唤醒 manager loop
3. `telegram_bot.py`：`/create` 后 touch `.wake_manager` — 新 run 立即被 manager 处理
4. Wake file 机制：零新依赖，幂等，和现有 fcntl lock 模式一致。Manager loop 每次 sleep 前检查，有信号则立即 tick

**D4.2 — 全局进度通知（已完成）：**
5. `telegram_bot.py`：`maybe_emit_progress_summary()` — 每 300s（可配置）推送全局进度
   - 格式：`[progress] 20 runs: 5 done, 3 failed, 8 active (5 executing, 3 iterating), 4 waiting (2 pushed, 2 ci wait).`
   - 变更检测：hash 比对，只在聚合状态变化时发送
   - Batch complete 检测：所有 run 到达终态或等待态时发 `[batch complete]`
   - NEEDS_HUMAN callout：有 run 需要人审时额外提醒

**D4.3 — 双 PR 模式 + Manager PR Readiness Assessment（已完成）：**
6. `manager_decision.py`：新增 `AUTO_CREATE_PR` action + `auto_pr_enabled` fact。PUSHED 状态逻辑：no review → RUN_CODE_REVIEW、HAS_ISSUES → RETRY(ITERATING)、CLEAN + auto → AUTO_CREATE_PR、CLEAN + manual → WAIT_HUMAN
7. `manager_llm.py`：新增 `PRReadinessResult` + `assess_pr_readiness()` + `_PR_READINESS_SYSTEM_PROMPT`。编码 7 条 review principles（reference provider alignment、minimal diff、no global pollution、dispatch integrity、PR body quality、PR template compliance、Worker creativity rule）。Safety override：code_ok/body_ok 任一 False → 强制 NEEDS_HUMAN
8. `manager_loop.py`：`_auto_create_pr()` 三步流程（request-open-pr → assess_pr_readiness → approve/escalate）+ `_run_pr_readiness_assessment()` 收集全部上下文（PR body、code review artifact、worker evidence、repo PR template、git diff）
9. `cli.py`：`--title` 变为可选，空时自动生成（`feat: add Forge as LLM provider for {repo_name}`）
10. `AGENTPR_AUTO_PR` 环境变量：`true`/`1`/`yes` 启用自动模式

**剩余：**
- Bot 会话持久化（run_id 绑定 SQLite）
- 自我迭代闭环（proposal → auto-apply skill patch → 人审批）
